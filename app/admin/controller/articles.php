...
class Articles extends \Core\Controller
{
    ...
    public function contentpublish()
    {
        ...
        $content = $className::where('articles_id', $item)/*->where('lang', $this->route->lang)*/
        ->first();
        $published = $content->published >= 1 ? 0 : 1;

        $className::where('articles_id', $item)/*->where('lang', $this->route->lang)*/
        ...
        $className::whereIn('articles_id', $ids)/*->where('lang', $this->route->lang)*/
        ->update(array('published' => $published));
